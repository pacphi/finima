...
- LoRA adapter weights do **not** survive restart — `ruvector-sona` 0.2.0 exposes export to safetensors but no import back into an engine. Accept that LoRA re-converges from recorded trajectories after boot.
...
ruvector-sona = { version = "0.2", default-features = false, features = ["serde-support"], optional = true }
...
>
> 1. **Crate naming correction.** The Rust crate is `ruvector-core` (not `ruvector`), and the self-learning crate is `ruvector-sona = "0.2.0"` — also on crates.io. The Phase 0 memo's earlier claim that SONA was unpublished was wrong.
> 2. **Version pin updated** from `0.1` to `2.1` for core; `0.2` for sona.
